...
try:
    ...
    collate = None
    collate_tensor_fn = None
    default_collate_fn_map = None
...
def custom_collate_fn(batch):
    if collate is None:
        raise ImportError(
            "custom_collate_fn requires PyTorch >= 1.13 "
            "(torch.utils.data._utils.collate.collate not found)"
        )
...
class TensorWrapper:
    ...
    def repeat_interleave(self, *args, **kwargs):
        return self.__class__(self._data.repeat_interleave(*args, **kwargs))

    def unique(self, dim=0):
        return self.__class__(self._data.unique(dim=dim))
    ...
    def half(self):
        return self.__class__(self._data.half())

    def bfloat16(self):
        return self.__class__(self._data.bfloat16())
    ...
    def where(cls, condition, input, other, *, out=None):
        data = torch.where(condition, input._data, other._data, out=out)
        return cls(data)
